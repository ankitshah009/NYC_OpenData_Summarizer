...
def rank_files_based_on_scores(directory_path):
    ...
    for rank, file_name in enumerate(sorted_files, 1):
        print(f"{rank}. {file_name} - Completeness: {scores[file_name]['completeness']:.2f}, Variability Avg: {scores[file_name]['variability_avg']:.2f}, Potential to have Bias: {len(scores[file_name]['bias_scores'])==1}")
